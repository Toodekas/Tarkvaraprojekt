...
class Graphs extends React.Component {
  ...
  handleChange2 = event => {
    const formValues2 = this.state.formValues2;
    const id = event.target.id || event.target.name;
    formValues2[id] = event.target.value;
  ...
  componentDidMount() {
    ...
    formValues2.alates = algus.toDateInputValue();
    formValues2.kuni = lopp.toDateInputValue();
    this.setState({ formValues2 });
  }
  ...
  getNewReport = (formValues) => {
    if (!isBrowser) {
      return false;
    }

    this.props.axios
      .get('generate_custom_report.php', {
        params: formValues,
      })
      .then(res => {
        this.setState({
        ...
        console.log('andmed: ', res);

        this.downloadCSV('custom_aruandlus');
  ...
  handleNewDownload = (event) => {
    event.preventDefault();
    this.getNewReport(this.state.formValues2);
  }
  ...
  render() {
    ...
    const makeDateField2 = (id, label) => (
      <FormControl required margin="normal" className={classes.formControl}>
        <TextField
          value={this.state.formValues2[id]}
          id={id}
          label={label}
          type="date"
    ...
    return (
      <Layout title="Aruandlus" error="">
        <div>
          <Paper>
            <form onSubmit={this.handleNewDownload}>
              {makeDateField2('alates', 'Alates')}
              {makeDateField2('kuni', 'Kuni')}
              <FormControl margin="normal">
                <InputLabel htmlFor="piirkond">Piirkond</InputLabel>
                <Select
                  value={this.state.formValues2.piirkond}
                  onChange={this.handleChange2}
                  id="piirkond"
                  inputProps={{
                    name: 'piirkond',
